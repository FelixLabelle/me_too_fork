...
for riot in riots:
    ...
    for source in sources:
        ...
        instances = [inst for inst in verb_insts if 'agent' in inst or 'patient' in inst]
        for current_entity in government_kw | protest_kw:
            current_instances = [instance for instance in instances if instance['verb_lemma'] == current_entity]
            if current_instances:
                agents = [entity for entity in current_instances if 'agent' in entity]
                patients = [entity for entity in current_instances if 'patient' in entity] 
                agent_scores = [[model.predict(agent['embedding']) - 1 for agent in agents] for model in models]
                patient_scores = [[model.predict(patient['embedding']) - 1 for patient in patients] for model in models]
                entity_scores[riot][source][gov_entity] = (patient_scores, agent_scores)
